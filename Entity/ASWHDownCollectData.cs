...
namespace Entity
{
    public class ASWHDownCollectData
    {
        private ASWHDownCollectData() { }
        private static ASWHDownCollectData instance;
        public static ASWHDownCollectData Instance
        {
            get
            {
                if (instance == null) instance = new ASWHDownCollectData();
                return instance;
            }
        }
        ...
        /// <param name="storeSite"></param>
        /// <param name="storeRoom"></param>
        public void DeleteCollectData(string matCode, string batchNo, string sn, string collectQty, string storeRoom, string storeSite, string trayNo)
        {
            for (int i = collect.Count - 1; i >= 0; i--)
            {
                if (collect[i].MatCode == matCode && collect[i].BatchNo == batchNo && collect[i].Sn == sn && collect[i].StoreSite == storeSite && collect[i].TrayNo == trayNo && collect[i].CollectQty == Convert.ToDecimal(collectQty))
                {
                    collect.RemoveAt(i);
                    return;
                }
